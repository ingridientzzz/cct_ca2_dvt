...
COLOR_BG = '#FFFFFF'
COLOR_TEXT = '#000000'
COLOR_PRIMARY = '#6699CC'
COLOR_SECONDARY = '#44AA99'
COLOR_MALE = '#88CCEE'
COLOR_FEMALE = '#806699'
COLOR_FEMALE_2011_DIST = '#A993BD'
...
def load_data(excel_file_path):
    ...
    try:
        ...
        if df_density_raw is None:
            st.error(f"Could not find 'MYE5_Table8' table in the Excel file: {excel_file_path}")
            df_density_raw = pd.DataFrame()
        if df_age_gender_raw is None:
            st.error(f"Could not find 'MYEB1_Table9' table in the Excel file: {excel_file_path}")
            df_age_gender_raw = pd.DataFrame()
...
df_density_wide = preprocess_density_data_wide(df_density_raw) if df_density_raw is not None and not df_density_raw.empty else pd.DataFrame()
df_density_melted = melt_density_data(df_density_wide) if not df_density_wide.empty else pd.DataFrame()
...
if df_display_density.empty:
    if df_density_raw is not None and df_density_raw.empty and not df_density_wide.empty:
...
st.markdown("---")
st.markdown(f"[By: ingridientzzz](https://github.com/ingridientzzz)")
