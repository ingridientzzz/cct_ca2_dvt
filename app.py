...
from streamlit_js_eval import streamlit_js_eval # Import the library

# Configs - FONT_FAMILY and trace colors can remain
FONT_FAMILY = '"Helvetica Neue", Helvetica, Arial, sans-serif'
# Okabe & Ito color-blind safe palette (for traces)
COLOR_PRIMARY = '#56B4E9'
...
# streamlit_js_eval to get the current theme from the browser
# will cause rerun when value is first fetched or changes
js_code = "document.documentElement.getAttribute('data-theme')"
theme_from_js = streamlit_js_eval(js_expressions=js_code, key="theme_detect")

if theme_from_js and theme_from_js != st.session_state.detected_theme:
    st.session_state.detected_theme = theme_from_js
    
# set Plotly template / colors based on Streamlit theme
current_plotly_template = "plotly_white" # default
plotly_paper_bgcolor = '#FFFFFF'
plotly_plot_bgcolor = '#FFFFFF' # plotly_white: transparent default plot_bgcolor
plotly_font_color = '#000000'

if st.session_state.detected_theme == "dark":
    current_plotly_template = "plotly_dark"
    # For plotly_dark, paper_bgcolor and plot_bgcolor usually dark
    # Font color is light, colors handled by template
    # for more specific control is needed:
    # plotly_paper_bgcolor = '#0E1117' # Streamlit's dark theme background
    # plotly_plot_bgcolor = '#0E1117'
    # plotly_font_color = '#FFFFFF'
else: # Light or None (default to light)
    current_plotly_template = "plotly_white"
    # plotly_paper_bgcolor = '#FFFFFF'
    # plotly_plot_bgcolor = '#FFFFFF'
    # plotly_font_color = '#000000'
...
def create_empty_figure(title_text):
    ...
    fig.update_layout(
        ...
        xaxis={'visible': False},
        yaxis={'visible': False},
        template=current_plotly_template, # Apply theme template
        # paper_bgcolor=plotly_paper_bgcolor, # Or set manually
        # plot_bgcolor=plotly_plot_bgcolor,
        font={'family': FONT_FAMILY} # font_color handled by template
    )
    return fig
...
st.markdown(f"Detected Streamlit Theme: **{st.session_state.detected_theme if st.session_state.detected_theme else 'Light (defaulting)'}**") # For debugging
st.markdown("---")

# --- Global Controls Section ---
st.sidebar.header("Global Filters")
global_selected_locations = st.sidebar.multiselect(
    "Select Geographic Location(s):", options=all_locations,
    default=['ENGLAND', 'SCOTLAND', 'WALES', 'NORTHERN IRELAND']
)
global_selected_year_display = st.sidebar.selectbox(
    "Select Year(s) to Display:",
    options=["Comparison (2011 & 2022)", "2011 Only", "2022 Only"], index=0
)
global_selected_gender_display = st.sidebar.selectbox(
    "Select Gender:", options=["Both", "Male", "Female"], index=0
)
global_selected_age_band = st.sidebar.selectbox(
    "Select Age Band:", options=age_bands_options, index=0
)
st.markdown("---")
...
if not global_selected_locations:
    ...
    if global_selected_year_display == "2011 Only" or global_selected_year_display == "Comparison (2011 & 2022)":
        fig_density.add_trace(go.Bar(
            x=filtered_df_density['name'], y=filtered_df_density['density_2011'], name='Density 2011',
    ...
    if global_selected_year_display == "2022 Only" or global_selected_year_display == "Comparison (2011 & 2022)":
        fig_density.add_trace(go.Bar(
            x=filtered_df_density['name'], y=filtered_df_density['density_2022'], name='Density 2022',
    ...
    fig_density.update_layout(
        ...
        xaxis={'categoryorder': 'array', 'categoryarray': sorted(global_selected_locations)},
        template=current_plotly_template, # Apply theme template
...
if loc_for_age_dist:
    genders_to_plot = []
    if global_selected_gender_display == "Male": genders_to_plot.append('M')
    elif global_selected_gender_display == "Female": genders_to_plot.append('F')
    elif global_selected_gender_display == "Both": genders_to_plot.extend(['M', 'F'])
    ...
    if not genders_to_plot:
            ...
            if not filtered_df_age_detail.empty:
                ...
                if global_selected_year_display == "2011 Only" or global_selected_year_display == "Comparison (2011 & 2022)":
                    fig_age_gender.add_trace(go.Scatter(
                        x=filtered_df_age_detail['age'], y=filtered_df_age_detail['population_2011'], name=f'{gender_label} 2011',
                        mode='lines+markers', marker_color=color_2011, line=dict(width=2, dash='dash'),
                        hovertemplate=(f"<b>{gender_label} - Age: %{{x}}</b><br>2011 Population: %{{y:,}}<extra></extra>")
                    ))
                if global_selected_year_display == "2022 Only" or global_selected_year_display == "Comparison (2011 & 2022)":
                    fig_age_gender.add_trace(go.Scatter(
                        x=filtered_df_age_detail['age'], y=filtered_df_age_detail['population_2022'], name=f'{gender_label} 2022',
                        mode='lines+markers', marker_color=color_2022, line=dict(width=2),
        ...
        if not data_found_for_age_dist:
            ...
            fig_age_gender.update_layout(
                ...
                xaxis={'type': 'category'}, hovermode='x unified', legend_title_text='Gender & Year',
                template=current_plotly_template, # Apply theme template
                font={'family': FONT_FAMILY},
                margin=dict(l=40, r=20, t=60, b=40)
            )
            if global_selected_age_band != 'All Ages':
                ...
                if x0_highlight is not None and x1_highlight is not None:
                    # vrect color should also adapt or be neutral
                    highlight_fillcolor = "rgba(128,128,128,0.2)" # Grey is usually okay on both
                    if st.session_state.detected_theme == "dark":
                        highlight_fillcolor = "rgba(180,180,180,0.15)" # Lighter grey for dark
                    fig_age_gender.add_vrect(
                        x0=x0_highlight, x1=x1_highlight,
                        fillcolor=highlight_fillcolor,
...
if global_selected_year_display == "2011 Only": year_for_gender_comp = 2011
elif global_selected_year_display == "2022 Only": year_for_gender_comp = 2022
    ...
    if grouped_df_gender.empty:
        ...
        if global_selected_gender_display == "Female" or global_selected_gender_display == "Both":
            ...
            if not df_female.empty:
                ...
                fig_gender_comp.add_trace(go.Bar(
                    x=df_female['name'], y=df_female['Population'], name='Female', marker_color=COLOR_FEMALE,
                    hovertemplate=(f"<b>%{{x}}</b><br>Females: %{{y:,}}<br>Year: {year_for_gender_comp}<br>Age Band: {global_selected_age_band}<extra></extra>")
                ))
        if global_selected_gender_display == "Male" or global_selected_gender_display == "Both":
            ...
            if not df_male.empty:
                data_added = True
                fig_gender_comp.add_trace(go.Bar(
                    x=df_male['name'], y=df_male['Population'], name='Male', marker_color=COLOR_MALE,
                    hovertemplate=(f"<b>%{{x}}</b><br>Males: %{{y:,}}<br>Year: {year_for_gender_comp}<br>Age Band: {global_selected_age_band}<extra></extra>")
                ))
        if not data_added:
