...
# Using the new color palette for theming
# Paul Tol Blue for primary accents, white background, black text.
ST_PRIMARY_COLOR = '#4477AA' # Paul Tol Blue
ST_BACKGROUND_COLOR = '#FFFFFF' # White
ST_SECONDARY_BACKGROUND_COLOR = '#F0F2F6' # Light Grey (often for sidebar/widget backgrounds)
ST_TEXT_COLOR = '#000000' # Black
ST_FONT = '"Helvetica Neue", Helvetica, Arial, sans-serif'

st.set_page_config(
    layout='wide',
    page_title="UK Population Dashboard", # Optional: Add a page title
    # page_icon="📊", # Optional: Add a page icon
    theme={
        "primaryColor": ST_PRIMARY_COLOR,
        "backgroundColor": ST_BACKGROUND_COLOR,
        "secondaryBackgroundColor": ST_SECONDARY_BACKGROUND_COLOR,
        "textColor": ST_TEXT_COLOR,
        "font": ST_FONT  # Can be "sans serif", "serif", or "monospace"
    }
)
...
# These are for the plots themselves
FONT_FAMILY = ST_FONT # Use the same font for consistency in plots
COLOR_BG = ST_BACKGROUND_COLOR # Plot background can match app background
COLOR_TEXT = ST_TEXT_COLOR     # Plot text can match app text
...
def create_empty_figure(title_text):
    ...
    fig.update_layout(
        ...
        xaxis={'visible': False},
        yaxis={'visible': False},
        font={'family': FONT_FAMILY, 'color': COLOR_TEXT}, # Use themed font and text color
        paper_bgcolor=COLOR_BG # Match plot background to app background
        )
    return fig
...
if df_display_density.empty:
    ...
    if not plot_data_exists_density: st.plotly_chart(create_empty_figure("No density data for selected year(s)/locations."), use_container_width=True)
        ...
        fig_density.update_layout(
            ...
            font={'family': FONT_FAMILY, 'color': COLOR_TEXT}, paper_bgcolor=COLOR_BG, plot_bgcolor=COLOR_BG,
...
st.st("Sex ratio is calculated as Males per 100 Females. This chart ignores the global 'Select Sex' filter.")
            ...
            if final_plot_data_sr.empty:
                ...
                if not plot_data_exists_sr:
                    ...
                    fig_sex_ratio_density.update_layout(
                        ...
                        font=dict(family=FONT_FAMILY, color=COLOR_TEXT), paper_bgcolor=COLOR_BG, plot_bgcolor=COLOR_BG,
